...
def check_2D_stability(rdmol, kekulize=False):
...
def smiles2mol(mol, sanitize=False):
    if not sanitize:
        params = Chem.SmilesParserParams()
        params.sanitize = False
        return Chem.MolFromSmiles(mol, params)
    else:
        return Chem.MolFromSmiles(mol)

def canonicalize_smiles(smiles, useChiral=1, sanitize=False):
    mol = smiles2mol(smiles, sanitize)
    return Chem.MolToSmiles(mol, useChiral)

def get_2D_edm_metric(predict_mols, train_mols=None, exist_fail_num=0):
    train_smiles = None
    ...
    ## scale the results using the addtion of failed molecules
    for key in output_dict.keys():
        output_dict[key] = output_dict[key] * len(predict_mols) / (len(predict_mols) + exist_fail_num)
    return output_dict
